...
<div class="card col-md-12">
    ...
    <div class="card-body">
        <form action="{{ route('admin.productcategory.update', Crypt::encryptString($item->id)) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('put')
            ...
            <div class="">
                ...
                <select  class=" selecttion form-control col-md-11 js-example-basic-single" name="product_sections_id" id="section">
                    ...
                    @foreach ($sections as $section)
                        <option {{ $section->id == $item->product_sections_id ? "selected":"" }} value="{{ $section->id }}">{{ $section->title }}</option>
                    @endforeach
                </select>
                @error('section')
