...
@section('main_content')
    <div class="card">
        <div class="card-header bg-primary">
            ...
            <a href="{{ route('admin.productcategory.create') }}" class="btn btn-light float-right"
                ...
                <tbody>
                    @foreach ($data as $item)
                        <tr>
                                ...
                                <div class="form-group">
                                    <div class="custom-control custom-switch">
                                        <input type="checkbox" class="custom-control-input statuschanger"
                                            status="productsubcategory" id="customSwitch{{ $item->id }}"
                            ...
                            <td class="d-flex">
                                <form method="post"
                                    action="{{ route('admin.productcategory.destroy', Crypt::encryptString($item->id)) }}">
                                    @csrf
                                    @method('delete')
                                ...
                                <a class="bg-primary p-3 ml-2"
                                    href="{{ route('admin.productcategory.edit', Crypt::encryptString($item->id)) }}"><i
                                        class="fa fa-pen"></i></a>
                            </td>
                        </tr>
                    @endforeach
