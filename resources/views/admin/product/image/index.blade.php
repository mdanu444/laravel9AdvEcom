...
<div class="card">
    ...
    <div class="card-body">
        <form action="{{ route('admin.p_image.store', Crypt::encryptString($product->id)) }}" method="post" enctype="multipart/form-data">
            @csrf
            @method('post')
            ...
            @foreach ($data as $item)
                <tr>
                    ...
                    <td class="p-3 text-center">
                        <img width="100" height="100" src="{{ url('images/product_image/alternative/'.$item->title) }}" alt="Product Image">
                    </td>
                    <td>
                        <form action="{{ route('admin.p_image.destroy', [Crypt::encryptString($product->id), Crypt::encryptString($item->id)]) }}" method="post">
                            @csrf
                            @method('delete')
